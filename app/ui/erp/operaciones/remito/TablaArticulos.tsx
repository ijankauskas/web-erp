...
export default function TablaArticulos({ register, articulos, setAlerta, setArticulos, bloquear, iva, cliente }: any) {
    ...
    const modificarArticulo = (e: React.ChangeEvent<HTMLInputElement>, index: any, field: string) => {
        ...
        const valueWithoutSeparators = value // Para convertir a número
                            ...
                            {articulos?.map((articulo: any, index: number) => (
                                <tr key={index} className={`${articulo.error ? 'bg-red-300' : ''} ${bloquear ? 'bg-yellow-100' : ''}`}>
                                    ...
                                    <td className="px-2 py-1 whitespace-nowrap text-sm text-gray-900 border border-gray-200 text-ellipsis overflow-hidden">
                                        <InputCommon
                                            tipo={'text'}
                                            texto={articulo.cantidad.toLocaleString()}
                                            id={`cantidad-${index}`}
                                            useForm={register(`cantidad-${index}`, {
                                                onChange: (e: React.ChangeEvent<HTMLInputElement>) => modificarArticulo(e, index, 'cantidad'),
                                                onBlur: (e: React.ChangeEvent<HTMLInputElement>) => formatearNumero(e, index, 'cantidad'),
                                            })}
                                            textAlign={'text-end'}
                                            paddingY={'py-0.5'}
                                            desactivado={bloquear}
                                    ...
                                    <td className="px-2 py-1 whitespace-nowrap text-sm text-gray-900 border border-gray-200 text-ellipsis overflow-hidden">
                                        <InputCommon
                                            tipo={'text'}
                                            texto={(articulo.precio_vta).toLocaleString()}
                                            id={`precio_vta-${index}`}
                                            useForm={register(`precio_vta-${index}`, {
                                                onChange: (e: React.ChangeEvent<HTMLInputElement>) => modificarArticulo(e, index, 'precio_vta'),
                                                onBlur: (e: React.ChangeEvent<HTMLInputElement>) => formatearNumero(e, index, 'precio_vta'),
                                            })}
                                            textAlign={'text-end'}
                                            paddingY={'py-0.5'}
                                            desactivado={bloquear}
                                    ...
                                    <td className="text-end px-3 py-1 whitespace-nowrap text-sm text-gray-900 border border-gray-200 text-ellipsis overflow-hidden">
                                        {(articulo.cantidad * articulo.precio_vta).toLocaleString()}
                                    </td>

                                    <td className="px-2 py-1 whitespace-nowrap text-sm text-gray-900 border border-gray-200 text-ellipsis overflow-hidden">
                                        <InputCommon
                                            tipo={'text'}
                                            texto={articulo.porcen_iva}
                                            id={`porcen_iva-${index}`}
                                            useForm={register(`porcen_iva-${index}`, {
                                                onChange: (e: React.ChangeEvent<HTMLInputElement>) => modificarArticulo(e, index, 'porcen_iva'),
                                                onBlur: (e: React.ChangeEvent<HTMLInputElement>) => formatearNumero(e, index, 'porcen_iva'),
                                            })}
                                            textAlign={'text-end'}
                                            paddingY={'py-0.5'}
                                            desactivado={bloquear}
